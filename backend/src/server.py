...
# --- INTEGRATED IMPORTS ---
# Import the sync router and background logic from your moved module
from src.Autonomous_State_Synchronization.github_webhook import github_router, process_webhook_logic
from src.token_store import get_valid_token
from src.Autonomous_State_Synchronization.jira_oauth import get_accessible_resources
from src.Autonomous_State_Synchronization.jira_oauth_router import jira_auth_router # <--- Add this
...
# Include the Jira OAuth and original webhook routes
app.include_router(github_router)
app.include_router(jira_auth_router) # <--- Add this
...
async def github_webhook(user_id: str, project_id: str, request: Request, background_tasks: BackgroundTasks):
    """
    Unified Handler: Updates code memory AND synchronizes Jira tasks in one request.
    """
    try:
        # 1. Fetch project safely from Supabase
        ...
        if "refs/heads/" in ref:
            ...
            # --- SUB-TASK 1: Update Digital Twin Code Memory ---
            ...
            if access_token:
                try:
                    ...
                    if resources:
                        ...
                        # Process Jira fulfillment in the background
                        background_tasks.add_task(
...
async def health_check():
    return {"status": "ok", "service": "Lumis Unified Project"}
